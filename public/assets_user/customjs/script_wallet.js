...
function transactionHistoryResponse(response) {
    ...
    if (history_list != null) {
        $.each(history_list, function (index, history) {
            ...
            var html = `<div class="d-flex justify-content-between border rounded-2 mb-2">
                            ...
                            <div class="pt-3 pe-3">
                                <span class="fw-bolder text-success"> + ₹${history.amount}</span><br>
                                <span class="fw-bold text-warning">${status_html}</span>
                            </div>
                        </div>`;
            $('#transactionhistorydiv').append(html);
        });
    }
